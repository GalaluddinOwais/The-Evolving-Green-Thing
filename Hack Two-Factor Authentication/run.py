...
def fitness(a, a1, a2):
    return physics.manifest(a, a1, a2)
...
def crossover(fitness_and_chromosome1, fitness_and_chromosome2, fitness_and_chromosome1_to_replace,
    ...
    random_gene_number = int(random.uniform(1, 2))
    # copy to crossover
...
def mutate(fitness_and_chromosome, fitness_and_chromosome_to_replace):
    ...
    random_genes_to_mutate = []
    for i in range(int(random.uniform(1, 3))):
        random_genes_to_mutate.append(int(random.uniform(0, 3)))

    for i in random_genes_to_mutate:
        if i == 0:
            chromosome_copy[1][i] = int(random.uniform(1, 20))
        else:
            chromosome_copy[1][i] = random.uniform(-FORCE, FORCE)
...
for _ in range(POPULATION):
    chromosomes.append([int(random.uniform(1, 20)), random.uniform(-FORCE, FORCE), random.uniform(-FORCE, FORCE)])
...
for chromosome in chromosomes:
    fitnessValue = fitness(chromosome[0], chromosome[1], chromosome[2])
