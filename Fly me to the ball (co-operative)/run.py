...
def mutate(fitness_and_chromosome, fitness_and_chromosome_to_replace, object_to_mutate):
    if object_to_mutate == 1:

        # copy
        chromosome_copy = copy.deepcopy(fitness_and_chromosome)
        # mutate a randomized gen
        list_of_random_genes_to_mutate = []
        for _ in range(int(random.uniform(1, 4))):  # random mutating energy hitting (1 to 6) alleles in a chromosome
        ...
        for random_gene_number in list_of_random_genes_to_mutate:
            if random_gene_number in [0, 3, 6]:
                chromosome_copy[1][random_gene_number] = int(random.uniform(1, 150))
            else:
    ...
    elif object_to_mutate == 2:

        # copy
        chromosome_copy = copy.deepcopy(fitness_and_chromosome)
        # mutate a randomized gen
        list_of_random_genes_to_mutate = []
        for _ in range(int(random.uniform(1, 4))):  # random mutating energy hitting (1 to 6) allels in a chromosome
            list_of_random_genes_to_mutate.append(int(random.uniform(10, 17)))

        for random_gene_number in list_of_random_genes_to_mutate:
            if random_gene_number in [9, 12, 15]:
                chromosome_copy[1][random_gene_number] = int(random.uniform(1, 150))
            else:
                chromosome_copy[1][random_gene_number] = random.uniform(-FORCE, FORCE)
...
for _ in range(POPULATION):
    chromosomes.append([int(random.uniform(1, 150)), random.uniform(-FORCE, FORCE), random.uniform(-FORCE, FORCE),
                        int(random.uniform(1, 150)), random.uniform(-FORCE, FORCE), random.uniform(-FORCE, FORCE),
                        int(random.uniform(1, 150)), random.uniform(-FORCE, FORCE), random.uniform(-FORCE, FORCE),
                        int(random.uniform(1, 150)), random.uniform(-FORCE, FORCE), random.uniform(-FORCE, FORCE),
                        int(random.uniform(1, 150)), random.uniform(-FORCE, FORCE), random.uniform(-FORCE, FORCE),
                        int(random.uniform(1, 150)), random.uniform(-FORCE, FORCE), random.uniform(-FORCE, FORCE)])
