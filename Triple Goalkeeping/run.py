...
def mutate(fitness_and_chromosome, fitness_and_chromosome_to_replace):
    ...
    for random_gene_number in list_of_random_genes_to_mutate:
        if random_gene_number in [0, 3, 6]:
            chromosome_copy[1][random_gene_number] = int(random.uniform(1, 85))
        else:
...
for _ in range(POPULATION):
    chromosomes.append([int(random.uniform(1, 85)), random.uniform(-FORCE, FORCE), random.uniform(-FORCE, FORCE),
                        int(random.uniform(1, 85)), random.uniform(-FORCE, FORCE), random.uniform(-FORCE, FORCE),
                        int(random.uniform(1, 85)), random.uniform(-FORCE, FORCE), random.uniform(-FORCE, FORCE)])
